var display = function(searched_database){
    ...
    $(columnh).append("Song Name")
    $(column1h).append("Album Name")
    $(column2h).append("Artist Names (used string_agg function ;))")
    $(column3h).append("Audio File Link")
    $(rowh).append(columnh)
    ...
    $.each(searched_database, function(i, value){
        ...
        while(x<(value.length)){
            ...
            $(column).append(value[x]["title"])
            $(column1).append(value[x]["album_name"])
            $(column2).append(value[x]["artist_names"])
            $(column3).append(value[x]["audio_file_link"])
...
var search_Lookup = function(song_id){
   $.ajax({
        type: "GET",
        url: "get_song_details?song_id=" + song_id,
        success: function(result){
            display(result)
...
var search_input = function ()
{
    ...
    $("#Search").attr("placeholder", "song_id").val("")
