var display = function(searched_database){
    ...
    $(columnh).append("Playlist Name")
    $(column1h).append("Songs in Playlist (used the string_agg function ;))")
    $(column2h).append("Date of Creation")
    $(column3h).append("Private?")
    $(rowh).append(columnh)
    ...
    $.each(searched_database, function(i, value){
        ...
        while(x<(value.length)){
            ...
            $(column).append(value[x]["playlist_name"])
            $(column1).append(value[x]["song_names"])
            $(column2).append(value[x]["date_of_creation"])
            $(column3).append(value[x]["private"])
...
var search_Lookup = function(playlist_id){
   $.ajax({
        type: "GET",
        url: "get_playlist_details?playlist_id=" + playlist_id,
        success: function(result){
            display(result)
        },
        error: function(request, status, error){
...
var search_input = function ()
{
    ...
    $("#Search").attr("placeholder", "playlist_id").val("")
