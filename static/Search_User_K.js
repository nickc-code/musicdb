var display = function(searched_database){
    ...
    $(columnh).append("Name of User")
    $(column1h).append("Username")
    $(column2h).append("Date of Joining")
    $(column3h).append("Artists Followed (This required the string_agg function ;))")
    $(rowh).append(columnh)
    ...
    $.each(searched_database, function(i, value){
        ...
        while(x<(value.length)){
            ...
            $(column1).append(value[x]["username"])
            $(column2).append(value[x]["date_of_joining"])
            $(column3).append(value[x]["artists_followed"])
...
var search_Lookup = function(playlist_id){
   $.ajax({
        type: "GET",
        url: "get_user_details?user_id=" + playlist_id,
        success: function(result){
            display(result)
        },
        error: function(request, status, error){
...
var search_input = function ()
{
    ...
    $("#Search").attr("placeholder", "user_id").val("")
